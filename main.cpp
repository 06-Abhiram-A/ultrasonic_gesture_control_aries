...
int main(int argc, char **argv) {
    ...
    int last_predicted_gesture_idx = -1;
    int consecutive_gesture_count = 0;
                ...
                case STATE_DETECTING:
                    {
                        ...
                        if (result.anomaly > 1.50f) { 
                            last_predicted_gesture_idx = -1;
                            consecutive_gesture_count = 0;
                            break; }
                        #endif
                        ...
                        //     }
                        // }
                        // --- SELECTIVE DEBOUNCE GATE ---
                        if (bv >= 0.75f && best != IDLE_CATEGORY_INDEX) {
                            // Update consecutive tracker
                            if (best == last_predicted_gesture_idx) {
                                consecutive_gesture_count++;
                            } else {
                                last_predicted_gesture_idx = best;
                                consecutive_gesture_count = 1;
                            }

                            bool trigger_now = false;

                            // RULE 1: Swipes are fast and transient. Trigger immediately on frame 1!
                            if (best == GESTURE_SWIPE) {
                                trigger_now = true;
                                ei_printf("DEBUG: Swipe detected! Triggering instantly (bypassing debounce).\n\r");
                            } 
                            // RULE 2: Pushes are sustained. Require 2 consecutive frames to prevent flickering.
                            else if (best == GESTURE_PUSH_LEFT || best == GESTURE_PUSH_RIGHT) {
                                if (consecutive_gesture_count >= 2) {
                                    trigger_now = true;
                                    ei_printf("DEBUG: Push stable for %d frames. Triggering.\n\r", consecutive_gesture_count);
                                } else {
                                    ei_printf("DEBUG: Push detected, waiting for stable window (1/2)...\n\r");
                                }
                            }

                            if (trigger_now) {
                                // --- HEURISTIC GUARD (Keep this to protect against physical blindspots) ---
                                bool left_sensor_saw_object = false;
                                for (int i = 0; i < WIN_SAMPLES; i++) {
                                    float left_sample_val = features[i * WIN_AXES + 0];
                                    if (left_sample_val < 42.0f) {
                                        left_sensor_saw_object = true;
                                    }
                                }

                                int corrected_gesture = best;
                                if (best == GESTURE_SWIPE && !left_sensor_saw_object) {
                                    ei_printf("GUARD: Overriding false Swipe to Push-Right (Left sensor was empty)\n\r");
                                    corrected_gesture = GESTURE_PUSH_RIGHT;
                                }
                                else if (best == GESTURE_PUSH_RIGHT && left_sensor_saw_object) {
                                    ei_printf("GUARD: Overriding false Push-Right to Swipe (Left sensor detected motion)\n\r");
                                    corrected_gesture = GESTURE_SWIPE;
                                }

                                confirmed_gesture_idx = corrected_gesture;
                                
                                // Reset debounce trackers for the next gesture
                                last_predicted_gesture_idx = -1;
                                consecutive_gesture_count = 0;
                            ...
                            // Drop to IDLE or low confidence resets the chain
                            last_predicted_gesture_idx = -1;
                            consecutive_gesture_count = 0;
                        }
                    }
